...
let Create = ()=>{
    fetch('json/base.json').then((data)=>{
        return data.json()
    }).then((fill)=>{
        const News = fill.News
        const massNews = Object.entries(News)
        massNews.forEach(item =>{
            if(item[1].img != undefined){
                const nav = document.querySelector('nav')
                const div = document.createElement('div')
                div.className = 'conteiner-news'
                div.innerHTML = `<div><img src="${item[1].img}" class="img-news"></div><br><div class="text-news"><p>${item[1].text}</p></div>`
                nav.append(div)
            }else{
                const nav = document.querySelector('nav')
                const div = document.createElement('div')
                div.className = 'conteiner-news'
                div.innerHTML = `<div class="text-news"><p>${item[1].text}</p></div>`
                nav.append(div)
                setTimeout(()=>{
                    let conteinerNews = document.querySelector('.conteiner-news')
                    conteinerNews.style.gridTemplateRows = `auto`
                }, 100)
            }
